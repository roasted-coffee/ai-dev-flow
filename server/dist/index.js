...
function findDefinition(name) {
    ...
    if (!db)
        return [];
    return db
        .prepare(`SELECT name, type, file, start_line, end_line
       FROM symbols
       WHERE name = ?
       ORDER BY CASE type
...
function findReferences(name, limit = 20) {
    const db = getDb();
    if (!db)
        return [];
    return db
        .prepare(`SELECT r.symbol_name, r.file, r.line,
...
function getContext(file, lineNum, radius = 40, level = 2, strip = false) {
    ...
    const enclosing = db
        ? db
            .prepare(`SELECT name, type, start_line, end_line
           FROM symbols
           WHERE file = ? AND start_line <= ? AND end_line >= ?
           ORDER BY (end_line - start_line) ASC
           LIMIT 1`)
            .get(file, lineNum, lineNum)
        : undefined;
...
function searchText(query, limit = 10) {
    const db = getDb();
    if (!db)
        return [];
    return db
        .prepare(`SELECT name, type, file, start_line
       FROM symbols
       WHERE name LIKE ?
       ORDER BY
...
function codeAction(symbol, intent) {
    const db = getDb();
    if (!db)
    ...
    const sym = db
        ...
        .get(symbol);
    if (!sym)
        return { error: `Symbol '${symbol}' not found in index` };
    const refs = db
        .prepare(`SELECT file, line FROM symbol_refs WHERE symbol_name = ? ORDER BY file, line LIMIT 20`)
        .all(symbol);
...
// ── New: analyze_usages ────────────────────────────────────────────────────────
function classifyPattern(line, symbol) {
    const t = line.trim();
    // Destructuring: a, b = ... (comma before =)
    if (/^[a-zA-Z_]\w*(?:\s*,\s*[a-zA-Z_]\w*)+\s*=/.test(t))
        return "destructuring";
    // Assignment: x = symbol(...)
    if (/^[a-zA-Z_][\w.]*\s*=\s*/.test(t) && t.includes(symbol))
        return "assignment";
...
function analyzeUsages(symbol) {
    ...
    const sym = db
        ...
        .get(symbol);
    const refs = db
        .prepare(`SELECT file, line FROM symbol_refs WHERE symbol_name = ? ORDER BY file, line LIMIT 100`)
        .all(symbol);
    const empty = {
        symbol,
    ...
    for (const ref of refs) {
        ...
        if (sym && ref.file === sym.file)
            groups.same_file.push(entry);
        else if (symDir && path.dirname(ref.file) === symDir)
            groups.same_module.push(entry);
        else
...
function generatePatch(symbol, intent) {
    const db = getDb();
    if (!db)
        return { error: "Database not loaded" };
    const sym = db
        .prepare(`SELECT name, type, file, start_line, end_line FROM symbols WHERE name = ? LIMIT 1`)
        .get(symbol);
    if (!sym)
        return { error: `Symbol '${symbol}' not found` };
    const fullPath = path.join(ROOT, sym.file);
    if (!fs.existsSync(fullPath))
        return { error: `File not found: ${sym.file}` };
    const allLines = fs.readFileSync(fullPath, "utf-8").split("\n");
...
// ── Server bootstrap ──────────────────────────────────────────────────────────
async function main() {
    const server = new Server({ name: "python-nav", version: "1.1.0" }, { capabilities: { tools: {} } });
        ...
        try {
            ...
            switch (name) {
                case "find_definition":
                    result = findDefinition(a.name);
                    break;
                case "find_references":
                    result = findReferences(a.name, a.limit ?? 20);
                    break;
                case "get_context":
                    result = getContext(a.file, a.line, a.radius ?? 40, a.level ?? 2, a.strip_comments ?? false);
                    break;
                case "search_text":
                    result = searchText(a.query, a.limit ?? 10);
                    break;
                case "code_action":
                    result = codeAction(a.symbol, a.intent);
                    break;
                case "analyze_usages":
                    result = analyzeUsages(a.symbol);
                    break;
                case "generate_patch":
                    result = generatePatch(a.symbol, a.intent);
                    break;
                default:
                    return {
